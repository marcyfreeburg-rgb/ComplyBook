...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Tax Report PDF generation - MUST be defined before /:organizationId/:year to avoid route collision
  ...
  // 1099 Form PDF generation - MUST be defined before /:organizationId/:year to avoid route collision
  ...
  app.get("/api/reports/form-990-schedule-a/:organizationId/:taxYear", isAuthenticated, async (req: any, res: Response) => {
    try {
      const userId = req.user.claims.sub;
      const organizationId = parseInt(req.params.organizationId);
      const taxYear = parseInt(req.params.taxYear);
...
Keep the response approximately 100-150 words.`;
      ...
      await storage.createAuditLog({
        ...
        entityType: "form990",
        entityId: narrativeType,
        oldValues: null,
        newValues: JSON.stringify({ narrativeType, taxYear }),
        ipAddress: req.ip || null,
        userAgent: req.headers["user-agent"] || null,
      });
