...
-   **Self-service registration:** `POST /api/register` endpoint (local auth mode only) allows new users to create accounts with email, password (with confirm), first name, last name, and optional organization name. Validates password strength (8+ chars, uppercase, lowercase, number), blocks disposable email domains, checks for enterprise invites in `PENDING_ENTERPRISE_INVITES`, auto-creates organization, sends notification email to tech@jandmsolutions.com, and auto-logs in the user after registration. Landing page CTAs link to `/login?signup=true` to show registration form.
-   **Password reset:** `POST /api/forgot-password` sends a reset email via SendGrid with a secure token (1-hour expiry). `POST /api/reset-password` validates the token, updates the password, and invalidates all existing sessions for that user. Frontend has a "Forgot password?" link on the login page and a `/reset-password?token=...` page for setting a new password. Tokens are stored in-memory (Map) with automatic expiry cleanup.
