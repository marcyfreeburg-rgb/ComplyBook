...
- **Operations Hub Cleanup**: Removed duplicate Bank Reconciliation tab (functionality exists in Bank/Accounting). Added organization type filtering for document categories (Grant Agreements and 990 Forms only visible to nonprofits). Integrated real document management API for listing and deleting documents. Upload dialog now guides users to upload from entity-specific pages (Contracts, Proposals, Change Orders) since documents must be attached to entities. Added STORAGE_PATH environment variable for Render production deployment.
- **Critical Multi-Tenant Data Isolation Fix**: Fixed a bug in Drizzle ORM queries where chaining `.where()` calls replaced previous conditions instead of combining them. This affected multiple functions: `getTaxReports`, `getTaxForm1099s`, `executeCustomReport` (transactions, invoices, bills, grants), `getAuditLogs`, `getTimeEntries`, `getProgramExpenses`, and `getProgramBudgetVsActual`. All functions now use `and(...conditions)` to properly combine organization/entity filters with optional date/status filters, preventing cross-organization data leakage.
- **AI Tax Deduction Analysis**: For-profit organizations can now use AI to analyze expense categories against IRS business tax deduction guidelines (Publications 334, 463, 535, and 587). The feature suggests which categories should be marked as tax-deductible, showing confidence scores and IRS category matches, with ability to apply suggestions individually or in bulk. All tax-related pages now include clear disclaimers that this is not tax advice and users should consult a qualified tax professional.
