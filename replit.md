...
- **Invoice Customization:** Complete visual customization system enabling organizations to brand their invoices with custom colors, fonts, and payment information. Features include: theme color selection (primary color for invoice heading/borders/totals, accent color for section headings); font family selection (Inter, Georgia, Arial, Times New Roman, Courier New); payment terms configuration (Net 30, Net 60, custom terms); payment methods configuration (wire transfer details, check instructions, credit card acceptance); custom footer text for branding and contact information; real-time preview showing all customizations; organization-level settings that apply to all invoices; and secure role-based access (owner/admin only). Settings persist to database and display in invoice previews with fresh data fetching on demand. Enables professional, branded invoices that match organizational identity.
