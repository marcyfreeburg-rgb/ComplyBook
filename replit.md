...
## Overview
Budget Manager is a web-based financial management platform for small non-profits and for-profit organizations. It enables users to manage multiple organizations, track income and expenses in multiple currencies, categorize transactions (with AI assistance), manage grants, plan budgets, and generate financial reports. The application provides a clean, professional, mobile-responsive interface inspired by leading financial dashboards.
...
- **Schema Design:** Multi-tenant with organization-scoped data; tables for users, organizations, user-organization roles, categories, transactions, grants, budgets, budget items, Plaid items, Plaid accounts, vendors, clients, currencies, categorization history, cash flow projections, cash flow assumptions, tax categories, tax reports, tax 1099 forms, expense approvals, and recurring transactions. All financial tables include currency fields (defaulting to USD) with foreign key references to the currencies table.
...
- **Multi-Currency Support (Backend):** Comprehensive multi-currency infrastructure enabling organizations to work with international transactions. Features include: dedicated currencies table with 10 pre-seeded currencies (USD, EUR, GBP, CAD, AUD, JPY, CNY, INR, MXN, BRL); exchange rate management with high precision (DECIMAL 12,6) storage to USD; organization-level default currency configuration; currency fields on all financial tables (transactions, invoices, bills, budgets, grants, recurring transactions, cash flow scenarios, tax reports, and expense approvals); RESTful API endpoints for currency retrieval and exchange rate updates; and role-based access control (admin/owner only for currency management). All currency codes follow ISO 4217 three-letter standard with foreign key constraints ensuring data integrity.
